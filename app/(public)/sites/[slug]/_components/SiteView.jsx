...
function PlotPopup({ plot, site, canManage, canEdit, onClose, onStatusSaved }) {
  ...
  return (
    <div className="relative w-[min(22rem,calc(100vw-2rem))] -translate-x-1/2 -translate-y-[calc(100%+14px)] rounded-lg border border-slate-200 bg-white shadow-2xl">
      <div className="flex items-start justify-between gap-4 rounded-t-lg bg-brand-navy p-4 text-white">
        <div>
          <p className="text-xs text-white/55">Plot</p>
          <h3 className="text-lg font-bold leading-tight">No. {plotNo}</h3>
          {props.Street_Nam && <p className="mt-0.5 text-xs text-white/60">{props.Street_Nam}</p>}
        </div>
        <button onClick={onClose} className="rounded-md p-1.5 text-white/70 transition-colors hover:bg-white/10 hover:text-white">
          <X className="h-4 w-4" />
        </button>
      </div>
      ...
      <div className="space-y-4 p-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {area > 0 && <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-600">{parseFloat(area).toFixed(3)} Acres</span>}
        </div>

        <div className="grid grid-cols-2 gap-3 text-sm">
          <PopupFact label="Price" value={price ? `GHS ${Number(price).toLocaleString()}` : "Contact us"} />
