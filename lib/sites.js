...
export function getSiteLabel(identifier) {
  const site =
    getSiteByTable(identifier) ??
    getSiteBySlug(identifier) ??
    SITES.find((item) => item.table.replaceAll("_", "-") === identifier) ??
    null;
  return site?.name ?? String(identifier ?? "");
}
